...
sub show 
{
	if($witem)
	{
		...
		tell application "iTunes"
			...
			set this_track to the track number of current track
			set this_year to the year of current track
		end tell
		return this_name & tab & this_artist & tab & this_rating & tab & this_length & tab & this_album & tab & this_stream & tab & this_time & tab & this_track & tab & this_year & tab
		);
		...
		$temp =~ s/\tmissing value$/\t/g;
		my ($track,$player,$rate,$duration,$album,$stream,$current_time,$track_number, $year) = split(/\t/,$temp);
...
#			$line = '"<track>" by "<player>" from the album "<album>" [<length>]';
			$line = '<player> [<album> (<year>)] <track_number>: <track> [<current_time> / <length>]';
			$line  =~ s/<track>/$track/gi;
			$line  =~ s/<player>/$player/gi;
			$line  =~ s/<length>/$duration/gi;
			$line  =~ s/<track_number>/$track_number/gi;
			$line  =~ s/<current_time>/$current_time/gi;
			$line  =~ s/<year>/$year/gi;
			if ($album)
			{
