...
cdef np.ndarray[DTYPE_t, ndim=1] find_ellipmom_1(
    ...
    cdef double detMinv = 1. / detM
    if (detM <= 0) + (Mxx <= 0) + (Myy <= 0):
        print("Error: non positive definite adaptive moments!\n")

    cdef double Minv_xx = Myy * detMinv
    cdef double TwoMinv_xy = -Mxy * detMinv * 2.0
    cdef double Minv_yy = Mxx * detMinv
    cdef double Inv2Minv_xx = 0.5 * detM / Myy  # Will be useful later...
    ...
    cdef double y_y0 = sqrt(max_moment_nsig2 * Myy)
    cdef double y1 = -y_y0 + y0
    cdef double y2 = y_y0 + y0

    # stay within image bounds
    cdef int iy1 = <int>ceil(y1)
    if iy1 < ymin:
        iy1 = ymin
    cdef int iy2 = <int>floor(y2)
    if iy2 > ymax:
        iy2 = ymax
    cdef int y

    cdef double a, b, c, d, sqrtd, inv2a, x1, x2, x_x0, \
...
cpdef np.ndarray[DTYPE_t, ndim=1] find_ellipmom_2(
        np.ndarray[DTYPE_t, ndim=2] data):

    cdef double epsilon = 1e-6
    cdef double convergence_factor = 1.0
    cdef double guess_sig = 3.0
    cdef double Mx = data.shape[1]
    cdef double My = data.shape[0]
    cdef double Mxx = guess_sig ** 2
    cdef double Mxy = 0
    cdef double Myy = guess_sig ** 2
    ...
    cdef double bound_correct_wt = 0.25  # Maximum shift in centroids and sigma between
                             # iterations for adaptive moments.

    cdef int num_iter = 0
    cdef int num_iter_max = 1000

    cdef np.ndarray[DTYPE_t, ndim=1] ellipmom_1
    ...
    cdef double two_psi, semi_a2, semi_b2, shiftscale, shiftscale0
    cdef dx, dy, dxx, dxy, dyy
    ...
    while (convergence_factor > epsilon) * (num_iter < num_iter_max):
        ...
        # Get moments
        ellipmom_1 = find_ellipmom_1(data, Mx, My,
                                     Mxx, Mxy, Myy,
                                     max_moment_nsig2)
        Amp = ellipmom_1[0]
        Bx = ellipmom_1[1]
        ...
        #print(Amp, Bx, By, Cxx, Cxy, Cyy, rho4)
    ...
    cdef np.ndarray[DTYPE_t, ndim=1] return_array = np.array([Mx, My, Mxx, Mxy, Myy, A, rho4], dtype=DTYPE)
    return return_array
