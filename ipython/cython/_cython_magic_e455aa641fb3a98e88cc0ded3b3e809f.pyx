...
cdef void find_ellipmom_1(np.ndarray[DTYPE_t, ndim=2] data,
                          float x0, float y0,
    ...
    cdef float a, b, c, d, sqrtd, inv2a, x1, x2, x_x0, Minv_xx__x_x0__x_x0, rho2, intensity
    cdef int ix1, ix2, x

    for y in xrange(iy1, iy2):

        y_y0 = <float>y - y0
        cdef float TwoMinv_xy__y_y0 = TwoMinv_xy * y_y0
        cdef float Minv_yy__y_y0__y_y0 = Minv_yy * y_y0 ** 2
    ...
    return A, Bx, By, Cxx, Cxy, Cyy, rho4w
