...
def find_ellipmom_1(data, x0, y0, Mxx, Mxy, Myy, max_moment_nsig2):

    A, Bx, By, Cxx, Cxy, Cyy, rho4w = [0] * 7
    ...
    y_y0 = np.sqrt(max_moment_nsig2 * Myy)
    y1 = -y_y0 + y0
    y2 = y_y0 + y0

    # stay within image bounds
    iy1 = max(int(ceil(y1)), ymin)
    iy2 = min(int(floor(y2)), ymax)
