...
cpdef int main():
    ...
    while coins[6] < max_coins[6]:
        ...
        max_coins[5] = (difference) / denominations[5]
        while coins[5] < max_coins[5]:
            for i in range(4 + 1):
                coins[i] = 0
            difference = target - value(coins, denominations)
            max_coins[4] = (difference) / denominations[4]
            while coins[4] < max_coins[4]:
                for i in range(3 + 1):
                    coins[i] = 0
                difference = target - value(coins, denominations)
                max_coins[3] = (difference) / denominations[3]
                while coins[3] < max_coins[3]:
                    for i in range(2 + 1):
                        coins[i] = 0
                    difference = target - value(coins, denominations)
                    max_coins[2] = (difference) / denominations[2]
                    while coins[2] < max_coins[2]:
                        for i in range(1 + 1):
                            coins[i] = 0
                        difference = target - value(coins, denominations)
                        max_coins[1] = (difference) / denominations[1]
